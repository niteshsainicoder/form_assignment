...
function FormComponent({ FormType }: {
    ...
    return (
        <form onSubmit={handleSubmit} className='flex w-[290px] sm:w-[400px] py-7 flex-col gap-5 border border-gray-500 rounded-lg p-5 justify-evenly items-center'>
            <h2>{FormType}</h2>
            <div className='w-full flex flex-wrap sm:flex-nowrap relative'>
                <label className='w-3/6 '>Name:</label>
            ...
            <div className='w-full flex flex-wrap sm:flex-nowrap relative'>
                <label className='max-w-3/6 w-3/6'>Country Code:</label>
                <select aria-label='countryCode' value={countryCode} onChange={(e) => setCountryCode(e.target.value)} className='bg-neutral-600 p-1 px-6 rounded-lg outline-none' >
